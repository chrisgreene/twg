...
func TestCampaignHandler_ShowActive(t *testing.T) {
	...
	tests := map[string]func(*testing.T) (*CampaignHandler, []checkFn){
		...
		"25 seconds left": func(t *testing.T) (*CampaignHandler, []checkFn) {
			mdb := &mockDB{
				ActiveCampaignFunc: func() (*db.Campaign, error) {
					return &db.Campaign{
						ID:       12,
		...
		"random sql error": func(t *testing.T) (*CampaignHandler, []checkFn) {
			...
			ch.TimeNow = timeNow
			lr := &logRecorder{}
			ch.Logger = lr // log.New(os.Stdout, "", log.LstdFlags)
			ch.Templates.Ended = template.Must(template.New("").Parse("Something went wrong..."))
			return &ch, checks(
				...
				func(t *testing.T, _ string) {
					...
					want := fmt.Sprintf("Error retrieving the active campaign. err = %v",
						sql.ErrConnDone)
					if lr.logs[0] != want {
...
func TestCampaignHandler_CampaignMw(t *testing.T) {
	...
	t.Run("campaign found", func(t *testing.T) {
		...
		handlerCalled := false
		gotPath := ""
		var gotCampaign *db.Campaign
		ch.DB = mdb
		handler := ch.CampaignMw(func(w http.ResponseWriter, r *http.Request) {
			handlerCalled = true
			gotPath = r.URL.Path
			gotCampaign = r.Context().Value("campaign").(*db.Campaign)
		})
		w := httptest.NewRecorder()
		r := httptest.NewRequest(http.MethodGet, "/123/id/here", nil)
		handler(w, r)
		...
		if gotCampaign != campaign {
			t.Fatalf("Campaign = %v; want %v", gotCampaign, campaign)
		}
	})
